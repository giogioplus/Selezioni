...
namespace SelezioniRep
{
    public static class DatiRep
    {
        ...
        {
            ds = new DataSet();
            ds.Tables.Add(DBUtility.DtRiepilogo(pSelId, pTipo, pCompId));
            ds.Tables.Add(DBUtility.DtScheda(pSelId, pTipo, pCompId));
            if (pCompId != 0)
                ds.Tables.Add(DBUtility.DtAnag(pSelId, pCompId));
            else
                ds.Tables.Add(DBUtility.DtAnag(pSelId, pUtente));
            return ds;
        }

        public static string EliminaLink(string pvalue)
        {
